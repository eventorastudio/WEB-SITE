const EVENT = Object.freeze({
  ...
  locationUrl: "https://www.google.com/maps/search/?api=1&query=Saltillo+Coahuila",
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('opening-guest').textContent = `${guestName} · Esta invitación es para ti.`;
  document.querySelectorAll('[data-guest-message]').forEach((element) => {
    element.textContent = `${guestName} · Esta invitación es para ti.`;
  });
  document.querySelectorAll('[data-pass-message]').forEach((element) => {
    element.textContent = passes === 1 ? "01 LUGAR RESERVADO" : `${String(passes).padStart(2, '0')} LUGARES RESERVADOS`;
  });
