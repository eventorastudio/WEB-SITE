...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('opening-guest').textContent = `Nos encantará compartir este día contigo, ${guestName}.`;
  document.querySelectorAll('[data-guest-message]').forEach((element) => {
    element.textContent = `Nos encantará compartir este día contigo, ${guestName}.`;
  });
  document.querySelectorAll('[data-pass-message]').forEach((element) => {
    element.textContent = passes === 1 ? "Hemos reservado un lugar para ti." : `${passes} lugares florecen en nuestra lista para ti.`;
  });
