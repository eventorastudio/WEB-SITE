const EVENT = Object.freeze({
  ...
  locationUrl: "https://www.google.com/maps/search/?api=1&query=Casa+Madero+Parras+Coahuila",
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('opening-guest').textContent = `Una invitación reservada especialmente para ${guestName}.`;
  document.querySelectorAll('[data-guest-message]').forEach((element) => {
    element.textContent = `Una invitación reservada especialmente para ${guestName}.`;
  });
  document.querySelectorAll('[data-pass-message]').forEach((element) => {
    element.textContent = passes === 1 ? "Un lugar reservado a tu nombre." : `${passes} lugares reservados a tu nombre.`;
  });
